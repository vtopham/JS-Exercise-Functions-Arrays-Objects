...
function makePersonObject(id, name, email) {
  let personObject = {id, name, email};
  return personObject;
  /* code here */
}
...
function getName(inputObject) {
  return `hello, my name is ${inputObject.name}`;
}
...
*/
function makeSmartPerson(inputName) {
  const returnObj = {
    name: inputName,
...
function getCarInfoById(inventory, id) {
  ...
  var findIndex = function(inventory, id) {
  ...
  var idIndex = findIndex(inventory, id);

  return `This is a ${inventory[idIndex].car_make} ${inventory[idIndex].car_model}`;
...
function sortCarInventory(inventory) {
  ...
  const sortByModel = function(inventory) {
    return inventory.sort(function(a,b) {
      const current = a.car_model;
      const compare = b.car_model;
...
function getModelYears(inventory) {
    const getYears = function(inventory) {
      const newArray = [];
...
function getOlderCars(inventory, maxYear) {
  const findByYear = function(inventory, maxYear) {
    const newList = [];
    for (var i = 0; i < inventory.length; i++) {
      if (inventory[i].car_year <= maxYear) {
        newList.push(inventory[i]);
      }
    }
    return newList;
  }
  return findByYear(inventory, maxYear);
}
...
function getGermanCars(inventory) {
  const findGerman = function(inventory) {
    const newArray = [];
    for (var i = 0; i < inventory.length; i++) {
...
function carMaker(odometerUpdate){
    
 const returnObj = {
    odometer: odometerUpdate,
    drive: function addMiles(od) {
      this.odometer += od;
